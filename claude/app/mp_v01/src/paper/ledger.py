"""Deterministic, restart-safe paper-account primitives. Never submits broker orders."""
...
class PaperPosition:
    ...
    def max_loss(self)->float: return round(self.quantity*self.entry_price*self.multiplier,2)

@dataclass(frozen=True)
class PaperAccount:
    equity:float; cash:float; positions:tuple[PaperPosition,...]=()
    def open_risk(self)->float: return round(sum(p.max_loss() for p in self.positions),2)
...
def proposal_context(account:PaperAccount, *, entry_price:float, quantity:int=1, multiplier:int=100)->dict[str,Any]:
    if account.equity<=0 or account.cash<0 or quantity<=0 or entry_price<=0 or multiplier<=0:
        raise ValueError("invalid paper-account or proposal values")
    ...
    return {"position_pct":proposed/account.equity,
            ...
            "open_positions":len(account.positions),"proposed_max_loss":round(proposed,2),
